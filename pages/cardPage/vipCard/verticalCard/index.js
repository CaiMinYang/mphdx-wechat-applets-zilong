...
Page({
  ...
  onLoad: function (options) {
    ...
    if (scene) {
      ...
      this.setData({
        skiptype: options.type, //0表示从名片夹进入 1表示分享到朋友圈之后查看的内容 2表示从群组中进入 3表示从交换页面进入 4表示从人脉集市页面进入  
    ...
    commont.postJson("/mp/carte/getCarteInfo", {
      ...
      if (res.code === '0000') {
        ...
        fieldAndDatas.forEach(function (item) {
          ...
          if (mp.field.source == 1 || mp.field.dataType == 1) {
            all.push(mp)
          } else {
            imgArr.push(mp)
          }
  ...
  selectComCard() {
    var djson = {
      id: this.data.cardid,
      companyid: this.data.cardInfo[0].id,
      companyname: this.data.cardInfo[0].companyname,
    }
    this.data.cardList.companyid = this.data.cardInfo[0].id;
    this.data.cardList.companyname = this.data.cardInfo[0].companyname;
    this.setData({
  ...
  editCard: commont.throttle(function (e) {
    wx.navigateTo({
      url: '/pages/cardPage/vipCard/editCard/index?companyid=' + this.data.companyid + '&from=vertical&id=' + this.data.cardid,
    })
  }, 200),
  ...
  onShareAppMessage: function (e) {
    ...
    if (e.target) {
      if (e.target.dataset.type == "giveCard") {
        ...
        if (feeType == 2) {
          if (shapeType == 1) {
            ...
            console.log('/pages/cardPage/vipCard/verticalCard/index?cardid=' + myCardid + "&type=1" + "&userid=" + _this.data.userInfo.userid + '&currentFace=1')
            return {
              title: _this.data.myforwardingScript || "您好,这是我的数字名片,请惠存！",
      ...
      console.log('/pages/cardPage/vipCard/previewCard/index?cardid=' + cardid + "&type=1" + "&userid=" + _this.data.creatorid + '&currentFace=' + currentFace)
      cardCommont.increaseSharednum(cardid, res => {})
      return {
        title: titles,
        path: '/pages/cardPage/vipCard/previewCard/index?cardid=' + cardid + "&type=1" + "&userid=" + _this.data.creatorid + '&currentFace=' + currentFace,
  ...
  changeFace() {
    ...
    // var all = fieldAndDatas.map(function(item){
    //   var mp = {
    //     ...item,
    //     at1: item.field.viewName.charAt(0) 
    //   }
    ...
    fieldAndDatas.forEach(function (item) {
      ...
      if (mp.item.field.source == 1 || mp.field.dataType == 1) {
        all.push(mp)
      } else {
        imgArr.push(mp)
      }
